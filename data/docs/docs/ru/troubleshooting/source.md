> Источник: https://yandex.ru/support/metrica/ru/troubleshooting/source
# Источники трафика и метки

Информация по определению источников переходов представлена в описании отчета [Источники, сводка](ru/sources/sources-summary).

Информацию о других рекламных системах Яндекс Метрика получает из меток — параметров в ссылке рекламного объявления. Если рекламная система не дополняет ссылку на ваш сайт меткой, вы можете самостоятельно сформировать ссылку с использованием меток `UTM` или `Openstat`. [Список меток](ru/reports/ad-systems)
...
- Если у вас есть доступ к редактированию ссылок на сайте-источнике, добавьте в ссылках метку [utm_referrer](ru/reports/tags) и в качестве значения укажите сайт-источник. [О группе отчетов «Метки»](ru/reports/tags)
